...
y_train = as.numeric(training_y)   # Ensure labels are 0-based

X_test = as.matrix(testing_x)
y_test = as.numeric(testing_y)   # Ensure labels are 0-based
...
clinical_cna_data_with_more_markers = na.omit(inner_join(clinical_cna_data, 
...
dim(clinical_cna_data_with_more_markers)
...
#Training Y
training_y = as.integer(as.factor(training_data$Overall.Survival.Status))-1   # Convert response variable to factor
...
#Testing Y
testing_y = as.integer(as.factor(testing_data$Overall.Survival.Status))-1  # True labels
